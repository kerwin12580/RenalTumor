...
T4.peaks <- read.table(
  file = "/data/raw_data/lzl/RenalTumor-20200713/cellRanger_result/scATAC-V1.2/T4/outs/peaks.bed",
  col.names = c("chr", "start", "end")
)
...
gr.T4 <- makeGRangesFromDataFrame(T4.peaks)

# Create a unified set of peaks to quantify in each dataset
combined.peaks <- reduce(x = c(gr.T2, gr.T3, gr.T4))
...
md.T4 <- read.table(
  file = "/data/raw_data/lzl/RenalTumor-20200713/cellRanger_result/scATAC-V1.2/T4/outs/singlecell.csv",
  stringsAsFactors = FALSE,
  sep = ",",
  header = TRUE,
  row.names = 1
...
md.T2 <- md.T2[md.T2$passed_filters > 500, ] # 59599*17
md.T3 <- md.T3[md.T3$passed_filters > 500, ] # 43387*17
md.T4 <- md.T4[md.T4$passed_filters > 500, ] # 43162*17
...
frags.T4 <- CreateFragmentObject(
  path = "/data/raw_data/lzl/RenalTumor-20200713/cellRanger_result/scATAC-V1.2/T4/outs/fragments.tsv.gz",
  cells = rownames(md.T4)
)
...
T4_assay <- CreateChromatinAssay(T4.counts, fragments = frags.T4)
T4 <- CreateSeuratObject(T4_assay, assay = "ATAC", meta.data = md.T4)
