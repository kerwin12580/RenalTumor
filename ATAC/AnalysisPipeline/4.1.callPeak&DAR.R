...
plot.cellType <- rev(c("CD4+ T cell", "Treg", "CD8+ T cell", "NK cell", "B cell", "Macrophage", "Monocyte", "Mast cell", "Endothelium (VCAM1+)", "Endothelium (VCAM1-)", "Mesangial cell", "Tumor"))
...
genes <- genes[-19]
pdf("2.Cluster/AnnotateCellType/cellType.coverage.plot.origin2.pdf")
...
pdf("2.Cluster/AnnotateCellType/cellType.coverage.plot2.pdf", width = unit(3, "inches"))
res <- sapply(genes, function(x){
  cat(x, "...\n")
  regions <- FindRegion(object = scATAC.data, region = x, assay = "Peaks", extend.upstream = 1000, extend.downstream = 1000)
  idx <- data.frame(findOverlaps(regions, promoter))
...
extra.info <- data.frame(gene = c("CD8A", "CD8A", "FGFBP2", "FGFBP2", "CSF1R", "FOXP3"),
                         region = c("chr2-86787200-86793401", "chr2-86805800-86810600", "chr4-15961400-15962601", "chr4-15963000-15963201", "chr5-150081600-150086801",
                                    "chrX-49268200-49271801"))
pdf("2.Cluster/AnnotateCellType/ensembl.pdf")
res <- sapply(unique(extra.info$gene), function(x){
  cat(x, "\n")
  idx <- which(extra.info$gene %in% x)
  regions <- StringToGRanges(extra.info$region[idx])
  p <- CoveragePlot(
    object = scATAC.data,
    region = x,
    ranges.title = "MACS2",
...
dev.off()                         
pdf("2.Cluster/AnnotateCellType/ensembl.small.pdf", width = unit(2.5, "inches"))
res <- sapply(unique(extra.info$gene), function(x){
  cat(x, "\n")
  idx <- which(extra.info$gene %in% x)
  regions <- StringToGRanges(extra.info$region[idx])
