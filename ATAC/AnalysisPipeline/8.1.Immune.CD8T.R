...
scATAC <- readRDS("sub.scATAC.harmony.pro.rds")
...
pdf("8.Immune/CD8T/Harmony.integration.pdf")
sub.scATAC.harmony <- Harmony.integration.reduceDimension(scATAC.object = sub.ATAC, set.resolutions = seq(0.2, 1.2, by = 0.1), groups = "dataset", assay = "ATAC", PC = 10, npcs = 20)
dev.off()
sub.scATAC.harmony$seurat_clusters <- sub.scATAC.harmony$ATAC_snn_res.0.3
...
pdf("8.Immune/CD8T/Integrate.scRNA.scATAC.pdf")
transfer.anchors1 <- Integrate.scRNA.scATAC(scATAC.object = sub.scATAC.harmony, scRNA.object = sub.scRNA.harmony,
                                            scATAC.assay = "ACTIVITY", ref.npcs = 20, dims = 20, PC = 10, scRNA.assay = "RNA",
                                            nfeatures = 3000, class.label = "cellType")
Coembedding1 <- Coembedding.scATAC.scRNA(transfer.anchors = transfer.anchors1$transfer.anchors, 
                                         ...
                                         scRNA.object = sub.scRNA.harmony, PC = 10, dims = 20, class.label = "cellType")
dev.off()

source(file = "/home/longzhilin/Analysis_Code/code/ratio.plot.R")
sub.scATAC.harmony$scRNA.id <- Coembedding1$scATAC.object$predicted.id
...
cellType3 <- gsub("^C1$", "Naive", cellType3)
...
sub.scATAC.harmony$cellType3 <- factor(cellType3, levels = c("Naive", "Tissue-resident", "Exhausted IEG.C1", "Exhausted IEG.C2", "Exhausted"))
...
####----------------------------------------------------- 4. marker activity
DefaultAssay(sub.scATAC.harmony) <- "ACTIVITY"
features <- c("LEF1", "IL7R", "TCF7", "TGFB1", "CD44", "CD69", "ZNF683", "ITGAE", "ITGA1",  
...
DotPlot(sub.scATAC.harmony, features = features, cols = c("#1e90ff", "#F15F30"), group.by = "cellType", dot.scale = 3.5) + NoLegend() + theme(axis.text.x = element_text(size = 7)) + theme(axis.text.y = element_text(size = 8))
dev.off()
...
DotPlot(sub.scATAC.harmony, features = TFs, cols = c("#1e90ff", "#F15F30"), group.by = "cellType", dot.scale = 3.5) + theme(axis.text.x = element_text(size = 7, angle = 90)) + theme(axis.text.y = element_text(size = 8))
DotPlot(sub.scATAC.harmony, features = TFs, cols = c("#1e90ff", "#F15F30"), group.by = "cellType", dot.scale = 3.5) + NoLegend() + theme(axis.text.x = element_text(size = 7, angle = 90)) + theme(axis.text.y = element_text(size = 8))
dev.off()
...
sub.scATAC.harmony <- RunChromVAR(
  object = sub.scATAC.harmony,
  genome = BSgenome.Hsapiens.UCSC.hg38
)
...
names(sub.motifs.chromVAR) <- idents
write.xlsx(sub.motifs.chromVAR, file = "8.Immune/CD8T/sub.motifs.chromVAR.human_pwms_v2.xlsx", sheetName = idents, rowNames = T)
...
sub.motifs.chromVAR.sig <- lapply(sub.motifs.chromVAR, function(x){
  x <- x %>% filter(avg_log2FC>0.25 & p_val_adj<0.05) %>% arrange(desc(avg_log2FC))
  return(x)
})
write.xlsx(sub.motifs.chromVAR.sig, file = "8.Immune/CD8T/sub.motifs.chromVAR.sig.human_pwms_v2.xlsx", sheetName = idents, rowNames = T)
...
top.TFs <- lapply(names(sub.motifs.chromVAR.sig), function(x){
    ...
    return(sub.motifs.chromVAR.sig[[x]]$gene[1:10])
})
...
top.TFs.res$Type <- factor(top.TFs.res$Type, levels = levels(sub.scATAC.harmony))
top.TFs.res$gene <- factor(top.TFs.res$gene , levels = unique(top.TFs.res$gene))
top.TFs.res$avg_log2FC <- top.TFs.res$avg_log2FC*0.8
library(ggthemes)
pdf("8.Immune/CD8T/top.TF.plot.pdf", width = 4)
p1 <- ggplot(top.TFs.res, 
            aes(x = Type, 
                ...
                color = p.adjust,
                fill = p.adjust))
p2 <- p1 + guides(color=FALSE) + geom_point(shape = 21) + theme_bw() + scale_size("avg_log2FC", breaks = c(0, 1, 2, 3), labels = c(0, 2, 4, 6), range = c(0, 4)) + scale_color_gradient("-log10(p_val_adj)", low = "#FF9375", high = "red") + scale_fill_gradient(low = "#FF9375", high = "red", breaks = c(1.3, 10, 30, 50, 70, 90), labels = c(0.05, 1e-10, 1e-30, 1e-50, 1e-70, 1e-90))
