...
pdf("5.Immune/CD8T/SCT.Harmony.Integration.pdf")
...
pi <- Combined.P.FC(cluster.DE[,c("avg_log2FC", "p_val_adj")], log10P = F)
cluster.DE$pi <- pi$pi
idents <- levels(sub.scRNA.harmony)
saveFormat <- lapply(idents, function(x){
  index <- which(cluster.DE$cluster == x)
  DEGs <- cluster.DE[index,]
  DEGs.up <- DEGs %>% filter(avg_log2FC>0) %>% arrange(desc(pi))
  DEGs.down <- DEGs %>% filter(avg_log2FC<0) %>% arrange(pi)
...
DEGs <- lapply(saveFormat, function(x){
    x <- x %>% filter(p_val_adj<0.05 & avg_log2FC>0.5) %>% arrange(desc(pi))
    return(x)
})
names(DEGs) <- idents
