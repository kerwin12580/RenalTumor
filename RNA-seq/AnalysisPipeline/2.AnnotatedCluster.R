...
cluster.label <- gsub("^0$", "NK cell", cluster.label)
...
plot.cellType <- c("CD4+ T cell", "Treg", "CD8+ T cell", "NK cell", "Proliferative CD8+ T cell", "B cell", "Macrophage", "Monocyte", "Dendritic cell", "Neutrophil", "Mast cell", "Endothelium (VCAM1+)", "Endothelium (VCAM1-)", "Mesangial cell", "Tumor")
...
pdf("2.Cluster/AnnotateCellType/cellType.marker.dotplot.pdf")
...
cellType.sig.DEGs <- cellType.all.markers %>% filter(avg_log2FC >=0.25 & p_val_adj < 0.05) %>% arrange(desc(pi))
saveFormat <- lapply(idents, function(x){
  index <- which(cellType.sig.DEGs$cluster == x)
  DEGs <- cellType.sig.DEGs[index,]
  DEGs <- DEGs %>% arrange(desc(pi))
  return(DEGs)
})
...
top.genes <- cellType.sig.DEGs %>% group_by(cluster) %>% top_n(n = 5, wt = pi)
