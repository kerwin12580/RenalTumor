...
T2 <- data.frame(mutation = c("VHL"),
                abundance = c(35.88), 
                Type = c("Frameshift indel"),
                patient = rep("T2", 1),stringsAsFactors = F)
T3 <- data.frame(mutation = c("VHL", "ARID1B"),
                abundance = c(37.78, 27.28), 
                Type = c("Missense", "Frameshift indel"),
                patient = rep("T3", 2), stringsAsFactors = F)
T4 <- data.frame(mutation = c("VHL", "TERT", "PTEN", "BARD1"),
                abundance = c(20.0, 17.59, 1.61, 1.35), 
                Type = c("Frameshift indel", "Missense", "Missense", "Missense"),
                patient = rep("T4", 4), stringsAsFactors = F)
#Because there are many mutations detected in T5, the mutation frequency is retained >2%; at the same time, VEGFA/CCND1 is added due to multiple mutations at multiple sites
T5 <- data.frame(mutation = c("VHL", "BAP1", "NF1", "MTOR", "COL5A3", "VEGFA"),
                ...
                patient = rep("T5", 6), stringsAsFactors = F)
all.mutation <- list(T2, T3, T4, T5)
