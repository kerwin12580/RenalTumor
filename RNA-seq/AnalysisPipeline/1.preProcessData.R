...
#36601*14644
T5.data <- Read10X(data.dir = "/data/raw_data/lzl/RenalTumor-20200713/cellRanger_result/scRNA-V5.0/T5/outs/filtered_feature_bc_matrix")
colnames(T5.data) <- paste0("T5_", colnames(T5.data))
#36601*11016
...
ggdensity(T5@meta.data, x = "nCount_RNA", title = "T5")
ggdensity(T5@meta.data, x = "nFeature_RNA", title = "T5")
ggdensity(T5@meta.data, x = "percent.mt", title = "T5")
dev.off()
...
T2.pro1 <- doubletDetect(Seurat.object = T2.pro, PCs = 1:50, doublet.rate = 0.061, annotation = "SCT_snn_res.0.7", sct = T) #7893 ~8000
T3.pro1 <- doubletDetect(Seurat.object = T3.pro, PCs = 1:50, doublet.rate = 0.106, annotation = "SCT_snn_res.1.7", sct = T) #13992 ~14000
T4.pro1 <- doubletDetect(Seurat.object = T4.pro, PCs = 1:50, doublet.rate = 0.091, annotation = "SCT_snn_res.0.7", sct = T) #11973 ~12000
T5.pro1 <- doubletDetect(Seurat.object = T5.pro, PCs = 1:50, doublet.rate = 0.061, annotation = "SCT_snn_res.0.5", sct = T) #8054 ~8000
dev.off()
saveRDS(T2.pro1, file = "T2.pro1.rds")
saveRDS(T3.pro1, file = "T3.pro1.rds")
saveRDS(T4.pro1, file = "T4.pro1.rds")
saveRDS(T5.pro1, file = "T5.pro1.rds")
...
T2.pro2 <- subset(T2.pro1, subset = Doublet == "Singlet") #21247*7449
T3.pro2 <- subset(T3.pro1, subset = Doublet == "Singlet") #21605*12574
T4.pro2 <- subset(T4.pro1, subset = Doublet == "Singlet") #21947*10937
T5.pro2 <- subset(T5.pro1, subset = Doublet == "Singlet") #20459*7605
saveRDS(T2.pro2, file = "T2.pro2.rds")
saveRDS(T3.pro2, file = "T3.pro2.rds")
saveRDS(T4.pro2, file = "T4.pro2.rds")
saveRDS(T5.pro2, file = "T5.pro2.rds")
...
Renal.list <- list(T2 = T2.pro2, T3 = T3.pro2, T4 = T4.pro2, T5 = T5.pro2)
