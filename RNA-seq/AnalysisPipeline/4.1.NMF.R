...
DefaultAssay(data.merge) <- "RNA"
T2.cancer.data <- subset(data.merge, subset = cellType == "Tumor" & orig.ident == "T2") # 606 cells
T2.cancer.data <- SCTransform(T2.cancer.data, vars.to.regress = c("nCount_RNA", "percent.mt"), return.only.var.genes = F, verbose = FALSE)

T3.cancer.data <- subset(data.merge, subset = cellType == "Tumor" & orig.ident == "T3") # 2382 cells
T3.cancer.data <- SCTransform(T3.cancer.data, vars.to.regress = c("nCount_RNA", "percent.mt"), return.only.var.genes = F, verbose = FALSE)

T4.cancer.data <- subset(data.merge, subset = cellType == "Tumor" & orig.ident == "T4") # 320 cells
T4.cancer.data <- SCTransform(T4.cancer.data, vars.to.regress = c("nCount_RNA", "percent.mt"), return.only.var.genes = F, verbose = FALSE)

T5.cancer.data <- subset(data.merge, subset = cellType == "Tumor" & orig.ident == "T5") # 256 cells
T5.cancer.data <- SCTransform(T5.cancer.data, vars.to.regress = c("nCount_RNA", "percent.mt"), return.only.var.genes = F, verbose = FALSE)
...
saveRDS(T5.cancer.data, file = "T5.cancer.data.rds")

#######################################################1.NMF calculate###################################################
T2.cancer.data <- readRDS("T2.cancer.data.rds") # 14896*606
T3.cancer.data <- readRDS("T3.cancer.data.rds") # 17153*2382
T4.cancer.data <- readRDS("T4.cancer.data.rds") # 14685*320
T5.cancer.data <- readRDS("T5.cancer.data.rds") # 12468*256
...
T2.cancer.scale.data <- Filter.gene(T2.cancer.scale.data) # 14634*606
T3.cancer.scale.data <- GetAssayData(T3.cancer.data, slot = "scale.data") 
T3.cancer.scale.data[which(T3.cancer.scale.data < 0)] <- 0 
T3.cancer.scale.data <- Filter.gene(T3.cancer.scale.data) # 16831*2382
T4.cancer.scale.data <- GetAssayData(T4.cancer.data, slot = "scale.data") 
T4.cancer.scale.data[which(T4.cancer.scale.data < 0)] <- 0
T4.cancer.scale.data <- Filter.gene(T4.cancer.scale.data) # 14390*320
T5.cancer.scale.data <- GetAssayData(T5.cancer.data, slot = "scale.data") 
T5.cancer.scale.data[which(T5.cancer.scale.data < 0)] <- 0 
T5.cancer.scale.data <- Filter.gene(T5.cancer.scale.data) # 12042*256
...
T5.sd <- apply(T5.cancer.scale.data, 1, sd)

sd.data <- data.frame(Sample = c(rep("T2", length(T2.sd)), rep("T3", length(T3.sd)),rep("T4", length(T4.sd)), rep("T5", length(T5.sd))),
                      sd = c(T2.sd, T3.sd, T4.sd, T5.sd))
...
T5.cancer.scale.data.pro <- T5.cancer.scale.data[names(T5.sd)[which(T5.sd >= 0.5)],]
...
T5.nmf <- nmf(T5.cancer.scale.data.pro, 2:6, seed=seed.data, nrun = 30, .options = "vP48")
saveRDS(T2.nmf, "T2.nmf.rds")
saveRDS(T3.nmf, "T3.nmf.rds")
saveRDS(T4.nmf, "T4.nmf.rds")
saveRDS(T5.nmf, "T5.nmf.rds")
...
plot(T5.nmf)
plot(2:6,T5.nmf$measures$cophenetic, type="b", col="purple")
dev.off()
...
T5.nmf.optimal <- nmf(T5.cancer.scale.data.pro, optimal.rank(T5.nmf), nrun = 30, seed=seed.data, .options = "vP48")
...
coefmap(T5.nmf.optimal)
basismap(T5.nmf.optimal)
consensusmap(T5.nmf.optimal)
dev.off()
...
saveRDS(T5.nmf.optimal, "T5.nmf.optimal.rds")
...
T5.cancer.data <- readRDS("T5.cancer.data.rds")
...
T5.nmf.optimal <- readRDS("T5.nmf.optimal.rds")
...
T5.w <- basis(T5.nmf.optimal)
T5.H <- coef(T5.nmf.optimal)

T2.basis.score <- as.numeric(T2.w)
T3.basis.score <- as.numeric(T3.w)
T4.basis.score <- as.numeric(T4.w)
T5.basis.score <- as.numeric(T5.w)
basis.score <- data.frame(patient = c(rep("T2", length(T2.basis.score)),
                                      rep("T3", length(T3.basis.score)),
                                      rep("T4", length(T4.basis.score)),
                                      rep("T5", length(T5.basis.score))), 
                          basis.score = c(T2.basis.score, T3.basis.score, T4.basis.score, T5.basis.score))
pdf("basis.score.pdf")
...
T5.program.orders <- extractFeatures(T5.nmf.optimal, 30)
T5.program.signature <- lapply(T5.program.orders, function(x){
    return(rownames(T5.w)[x])
})
...
colnames(T5.w) <- paste0("T5_Program", 1:ncol(T5.w))
NMF.score <- list(T2 = T2.w, T3 = T3.w, T4 = T4.w, T5 = T5.w)
...
T5.data.scale <- GetAssayData(T5.cancer.data, slot = "scale.data")
...
anno.info <- data.frame(Type = rep(paste0("Program", 1:length(T5.program.orders)), sapply(T5.program.orders, function(x){length(x)})))
row.anno <- HeatmapAnnotation(Type = anno.info$Type, which = "row", col = row.list)
top.anno <- HeatmapAnnotation(Phase = T5.cancer.data$Phase, 
                              col = col.list)
p <- Heatmap(T5.data.scale[unlist(T5.program.signature),], name = "Expression", cluster_rows = F, top_annotation = top.anno, left_annotation = row.anno, show_row_dend = F, show_column_dend = F, show_row_names = F, show_column_names = F)
print(p)
dev.off()
...
names(T5.program.signature) <- paste0("T5_Program", 1:length(T5.program.signature))
All.program.signature <- c(T2.program.signature, T3.program.signature, T4.program.signature, T5.program.signature)
...
T5.cancer.data <- AddModuleScore(T5.cancer.data, features = All.program.signature, name = paste0(names(All.program.signature), "_"))
T5.program.score <- T5.cancer.data@meta.data[, grep("Program", colnames(T5.cancer.data@meta.data))]
colnames(T5.program.score) <- gsub("_\\d+", "", colnames(T5.program.score))
single.programScore <- list(T2 = T2.program.score, T3 = T3.program.score, T4 = T4.program.score, T5 = T5.program.score)
...
T5.corrM <- cor(T5.program.score)
corrplot(T5.corrM, order = "hclust", col=colorRampPalette(c("blue","white","#EA4335"))(100), hclust.method = "ward.D", title = "T5", addCoef.col = "black", number.cex = 0.75, number.font = 3, tl.col = "black", tl.cex = 0.8)
mean.corrM <- (T2.corrM+T3.corrM+T4.corrM+T5.corrM)/4
...
cluster.order <- sapply(c("T2_", "T3_", "T4_", "T5_"), function(x){
...
colnames(program.matrix) <- c("T2", "T3", "T4", "T5")
...
res <- singleScore.cluster(program.score = T5.program.score, cluster.order = cluster.order, row_split = 5)
...
metaProgram1 <- c("T3_Program2", "T2_Program1", "T4_Program1", "T5_Program1")
metaProgram2 <- c("T5_Program2", "T3_Program1", "T4_Program3")
#metaProgram3 <- c("T3_Program3", "T4_Program2")
...
enrich <- list(MetaProgram1 = enrich1[c(1:3,7,9,12,14,20,21,27,33,77,131,141,165), c("ID", "Count", "p.adjust", "geneID")], #
